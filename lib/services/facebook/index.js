...
const getData = async (request, _res=[]) => {
  ...
  // Call facebook SDK
  const apiRes = await FB.api(request.endpoint, request.method, request.params);
  if(!apiRes || apiRes.error) {
    throw new Error(!res ? 'error occurred' : apiRes.error);
  }
  // Recursive block
  if (totalLimit && apiRes.paging) {
    ...
    _res = _res.concat(apiRes.data)
    if (totalLimit > 0 && _res.length >= totalLimit) {
      return _res.splice(0, totalLimit)
